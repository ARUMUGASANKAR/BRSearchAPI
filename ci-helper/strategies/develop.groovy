def call(isMerge, prNumber, workDir='') {
    ...
    properties([
      parameters([
          buildTagParameter("TAG_TO_DEPLOY", "Repository Tags", "${repo}", "develop/pr/"),
          buildChoiceParameter("ENV_STACK", "Environments", '["dev"]')
      ])
    ])
...
//                        }
                    ...
                    if ((!isManual) || (isManual && env.TAG_TO_DEPLOY && env.ENV_STACK)) { 
                        ...
                        manager.addShortText("Not all parameters were selected","black", "yellow", "1px", "grey"  )
                ...
                if(env.ENV_STACK) {
                    ...
                    currentBuild.description = "${p.applicationUrl}/SearchAssemblerService <br>docker pull ${p.dockerRegistryUrl.split('/')[2]}/${fullAppTag}"
